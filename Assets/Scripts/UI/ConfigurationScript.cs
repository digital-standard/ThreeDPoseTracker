...
public class ConfigurationScript : MonoBehaviour
{
    ...
    private GameObject pnlWindowCapture;
    ...
    private InputField ifKalmanFilterInterval;
    private InputField ifKalmanFilterNoise;
...
*/
    ...
    private Toggle UseWindowCapture;
    private Dropdown ddProcessSelect;
    private InputField ifWCX;
    private InputField ifWCY;
    private InputField ifWCZ;
    private InputField ifWCRotX;
    private InputField ifWCRotY;
    private InputField ifWCRotZ;
    private InputField ifWCScale;
    private InputField ifWCFrameRate;
    private Toggle WCDrawCursor;
    ...
    public void Init()
    {
        ...
        pnlWindowCapture = GameObject.Find("pnlWindowCapture");
        ...
        ifKalmanFilterInterval = GameObject.Find("ifKalmanFilterInterval").GetComponent<InputField>();
        ifKalmanFilterNoise = GameObject.Find("ifKalmanFilterNoise").GetComponent<InputField>();
...
*/
        ...
        UseWindowCapture = GameObject.Find("UseWindowCapture").GetComponent<Toggle>();
        ddProcessSelect = GameObject.Find("ProcessSelect").GetComponent<Dropdown>();
        ifWCX = GameObject.Find("ifWCPosX").GetComponent<InputField>();
        ifWCY = GameObject.Find("ifWCPosY").GetComponent<InputField>();
        ifWCZ = GameObject.Find("ifWCPosZ").GetComponent<InputField>();
        ifWCRotX = GameObject.Find("ifWCRotX").GetComponent<InputField>();
        ifWCRotY = GameObject.Find("ifWCRotY").GetComponent<InputField>();
        ifWCRotZ = GameObject.Find("ifWCRotZ").GetComponent<InputField>();
        ifWCScale = GameObject.Find("ifWCScale").GetComponent<InputField>();
        ifWCFrameRate = GameObject.Find("ifWCFrameRate").GetComponent<InputField>();
        WCDrawCursor = GameObject.Find("WCDrawCursor").GetComponent<Toggle>();

        ddProcessSelect.ClearOptions();
    ...
    public void ShowSetting(ConfigurationSetting config)
    {
        ...
        ifKalmanFilterInterval.text = config.KalmanFilterInterval.ToString("0.00");
        ifKalmanFilterNoise.text = config.KalmanFilterNoise.ToString("0.00");
...
*/
        ...
        UseWindowCapture.isOn = config.UseWindowCapture == 1;
        ifWCX.text = config.WCPosX.ToString("0.00");
        ifWCY.text = config.WCPosY.ToString("0.00");
        ifWCZ.text = config.WCPosZ.ToString("0.00");
        ifWCRotX.text = config.WCRotX.ToString("0.00");
        ifWCRotY.text = config.WCRotY.ToString("0.00");
        ifWCRotZ.text = config.WCRotZ.ToString("0.00");
        ifWCScale.text = config.WCScale.ToString("0.00");
        ifWCFrameRate.text = config.WCFrameRate.ToString("0");
        WCDrawCursor.isOn = config.WCDrawCursor == 1;
    ...
    private string SetSetting()
    {
        ...
        if (!float.TryParse(ifKalmanFilterInterval.text, out f))
        {
            return "Kalman Filter Interval is required.";
        }
        if (f < 0f || f > 1f)
        {
            return "Kalman Filter Interval is between 0 and 1.";
        }
        configurationSetting.KalmanFilterInterval = f;
        if (!float.TryParse(ifKalmanFilterNoise.text, out f))
        {
            return "Kalman Filter Noise is required.";
        }
        if (f < 0f || f > 1f)
        {
            return "Kalman Filter Noise is between 0 and 1.";
        }
        configurationSetting.KalmanFilterNoise = f;
        ...
        */
        ...
        configurationSetting.UseWindowCapture = UseWindowCapture.isOn ? 1 : 0;
        configurationSetting.WindowTitle = ddProcessSelect.options[ddProcessSelect.value].text;
        if (!float.TryParse(ifWCX.text, out f))
        {
            return "Window Capture Position X is required.";
        }
        if (f < -100f || f > 100f)
        {
            return "Window Capture Position X is between -100 and 100.";
        }
        configurationSetting.WCPosX = f;
        if (!float.TryParse(ifWCY.text, out f))
        {
            return "Window Capture Position Y is required.";
        }
        if (f < -100f || f > 100f)
        {
            return "Window Capture Position Y is between -100 and 100.";
        }
        configurationSetting.WCPosY = f;
        if (!float.TryParse(ifWCZ.text, out f))
        {
            return "Window Capture Position Z is required.";
        }
        if (f < -100f || f > 100f)
        {
            return "Window Capture Position Z is between -100 and 100.";
        }
        configurationSetting.WCPosZ = f;
        if (!float.TryParse(ifWCRotX.text, out f))
        {
            return "Window Capture Rotation X is required.";
        }
        if (f < -100f || f > 100f)
        {
            return "Window Capture Rotation X is between -100 and 100.";
        }
        configurationSetting.WCRotX = f;
        if (!float.TryParse(ifWCRotY.text, out f))
        {
            return "Room Rotation Y is required.";
        }
        if (f < -100f || f > 100f)
        {
            return "Window Capture Rotation Y is between -100 and 100.";
        }
        configurationSetting.WCRotY = f;
        if (!float.TryParse(ifWCRotZ.text, out f))
        {
            return "Window Capture Rotation Z is required.";
        }
        if (f < -100f || f > 100f)
        {
            return "Window Capture Rotation Z is between -100 and 100.";
        }
        configurationSetting.WCRotZ = f;
        if (!float.TryParse(ifWCScale.text, out f))
        {
            return "Window Capture Scale X is required.";
        }
        if (f <= 0f || f > 100f)
        {
            return "Window Capture Scale is between 0 and 100.";
        }
        configurationSetting.WCScale = f;
        if (!int.TryParse(ifWCFrameRate.text, out i))
        {
            return "Window Capture Frame Rate is required.";
        }
        if (f <= 0f || f >= 100f)
        {
            return "Window Capture Frame Rate is between 0 and 100.";
        }
        configurationSetting.WCFrameRate = i;
        configurationSetting.WCDrawCursor = WCDrawCursor.isOn ? 1 : 0;
    ...
    public void onTabWindowCapture()
    {
        DeactivateTabPanel();
        pnlWindowCapture.SetActive(true);
    }
    ...
    private void DeactivateTabPanel()
    {
        ...
        if (pnlWindowCapture.activeSelf) pnlWindowCapture.SetActive(false);
    ...
    void OnWindowAdded(UwcWindow window)
    {
        ...
        ddProcessSelect.options.Add(new Dropdown.OptionData(window.title));
