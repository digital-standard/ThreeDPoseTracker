...
public class UIScript : MonoBehaviour
{
    ...
    public GameObject WindowCapture;
    ...
    private void SetWindowCapture(ConfigurationSetting config)
    {
        if(config.UseWindowCapture == 0)
        {
        ...
        else
        {
            ...
            WindowCapture.transform.position = new Vector3(config.WCPosX, config.WCPosY + 1.7f, config.WCPosZ + 1f);
            WindowCapture.transform.eulerAngles = new Vector3(config.WCRotX, config.WCRotY, config.WCRotZ);
            var scrt = WindowCapture.GetComponent<uWindowCapture.UwcWindowTexture>();
            scrt.partialWindowTitle = config.WindowTitle;
            scrt.scalePer1000Pixel = config.WCScale;
            scrt.captureFrameRate = config.WCFrameRate;
            scrt.drawCursor = config.WCDrawCursor == 1;
        }
    }

    private void SetVMCProtocol(ConfigurationSetting config)
    {
    ...
    private void SetRoom(ConfigurationSetting config)
    {
        if (config.ShowRoom == 1)
        {
            Room.SetActive(true);
            SetRoomSetting();
            if (config.ReceiveShadow == 1)
            {
    ...
    private void ReflectConfiguration(ConfigurationSetting config)
    {
        ...
        SetWindowCapture(config);
        SetVMCProtocol(config);
...
public class ConfigurationSetting
{
    ...
    public float KalmanFilterInterval;
    public float KalmanFilterNoise;
...
*/
    ...
    public float WCRotY;
    public float WCRotZ;
    public float WCScale;
    public int WCFrameRate;
    public int WCDrawCursor;
    ...
    public ConfigurationSetting()
    {
        ...
        KalmanFilterInterval = 0.45f;
        KalmanFilterNoise = 0.4f;
        LowPassFilter = 0.1f;
        NOrderLPF = 7;
        //BWBuffer = 100;
        //BWCutoff = 10.0f;
...
*/
    ...
    public ConfigurationSetting Clone()
    {
        return new ConfigurationSetting()
        {
            ...
            KalmanFilterInterval = KalmanFilterInterval,
            KalmanFilterNoise = KalmanFilterNoise,
...
*/
            ...
            UseWindowCapture = UseWindowCapture,
            ...
            WCRotZ = WCRotZ,
            WCScale = WCScale,
            WCFrameRate = WCFrameRate,
            WCDrawCursor = WCDrawCursor,
    ...
    public void Load()
    {
        ...
        KalmanFilterInterval = PlayerPrefs.GetFloat("KalmanFilterInterval", KalmanFilterInterval);
        KalmanFilterNoise = PlayerPrefs.GetFloat("KalmanFilterNoise", KalmanFilterNoise);
...
*/
        ...
        UseWindowCapture = PlayerPrefs.GetInt("UseWindowCapture", UseWindowCapture);
        WCPosX = PlayerPrefs.GetFloat("WCPosX", WCPosX);
        WCPosY = PlayerPrefs.GetFloat("WCPosY", WCPosY);
        WCPosZ = PlayerPrefs.GetFloat("WCPosZ", WCPosZ);
        WCRotX = PlayerPrefs.GetFloat("WCRotX", WCRotX);
        WCRotY = PlayerPrefs.GetFloat("WCRotY", WCRotY);
        WCRotZ = PlayerPrefs.GetFloat("WCRotZ", WCRotZ);
        WCScale = PlayerPrefs.GetFloat("WCScale", WCScale);
        WCFrameRate = PlayerPrefs.GetInt("WCFrameRate", WCFrameRate);
        WCDrawCursor = PlayerPrefs.GetInt("WCDrawCursor", WCDrawCursor);
    ...
    public void Save()
    {
        ...
        ppSet("KalmanFilterInterval", KalmanFilterInterval);
        ppSet("KalmanFilterNoise", KalmanFilterNoise);
...
*/
        ...
        ppSet("UseWindowCapture", UseWindowCapture);
        ppSet("WCPosX", WCPosX);
        ppSet("WCPosY", WCPosY);
        ppSet("WCPosZ", WCPosZ);
        ppSet("WCRotX", WCRotX);
        ppSet("WCRotY", WCRotY);
        ppSet("WCRotZ", WCRotZ);
        ppSet("WCScale", WCScale);
        ppSet("WCFrameRate", WCFrameRate);
        ppSet("WCDrawCursor", WCDrawCursor);
